...
void AstDumper::visit(ConstantValueNode &p_constant_value) {
    ...
    ConstValue *constval = p_constant_value.getConstantValue();
    std::string str;
    switch(constval->val_type) {
        case TypeEnum::TYPE_INT:
            str = std::to_string(constval->int_val);
            break;
        case TypeEnum::TYPE_REAL:
            str = std::to_string(constval->real_val);
            break;
        case TypeEnum::TYPE_STR:
            str = (const char *)constval->str_val;
            break;
        case TypeEnum::TYPE_BOOL:
            str = constval->bool_val;
            break;
        default:
            break;
    }
